...
const float ki = 1.0; // [V/(rad*s)]
const float kd = 1.0; // [V/(rad/s)]
...
const byte REN_OUTPUT[]  = {    22,    24,    26,    28,    30,    32}; // PortA
const byte LEN_OUTPUT[]  = {    23,    25,    27,    29,    31,    33}; // PortB
...
void setup() {
  ...
  long temp[6];
  getCurrentCounts(temp);
  getDesiredEncoderCounts(desiredCounts);
  vecSub(error, desiredCounts, temp, 6);
...
void loop() {
  ...
  for (int n = 0; n < runsPerRead; n++) {
    for (int m = 0; m < 6; m++) {
      ...
      long ctrlSig = kp * counts2rad(error[m], countsPerRev) + kd * counts2rad(de_dt, countsPerRev) + ki * counts2rad(intError[m], countsPerRev);
...
boolean controlled_move(float desired[6], long lastEnd[6], long lastCnts[6], long current[6], byte motorOn[6], unsigned int iterations) {
  ...
  for (int n = 0; n < iterations; n++) {
    for (int m = 0; m < 6; m++) {
      ...
      elapsedError = current[m] - lastCnts[m];
      ...
      unsigned int PWMvalue = (int)constrain(abs(ctrlSig / 5 * 255), 0, 100); // Heavily constrained for safety, usually goes to 255
...
*/
void getDesiredEncoderCounts(long * counts) {
